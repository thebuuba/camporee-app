...
export default async function MealsPage() {
  ...
  const { data: meals, error: mealsError } = camporee ? await supabase.from("meals").select("id,meal_date,meal_type,menu,responsible_name,notes").eq("camporee_id", camporee.id).order("meal_date").order("meal_type") : { data: [], error: null };
  const contentError = mealsError;
