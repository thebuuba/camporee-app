...
export default function MealManager({ camporeeId, canEdit, canEditLists, initialMeals, inventory, initialLists }: { camporeeId: string; canEdit: boolean; canEditLists:boolean; initialMeals: any[]; inventory:any[]; initialLists:any[] }) {
  ...
  const visible = useMemo(() => meals.filter((meal) => {
    const ingredientText=(meal.meal_ingredients||[]).map((item:any)=>item.name).join(' ');
    const matchesText = `${mealLabel(meal.meal_type)} ${meal.menu} ${meal.responsible_name || ""} ${ingredientText}`.toLowerCase().includes(query.toLowerCase());
    return matchesText && (filter === "all" || meal.meal_type === filter);
  }), [meals, query, filter]);

  function openNew() { setEditing(null); setOpen(true); }
  function openEdit(meal: any) { setEditing(meal); setOpen(true); }
  function shortage(ingredient:any){
    const stock = ingredient.inventory_item_id ? inventoryMap.get(ingredient.inventory_item_id) : null;
    return Math.max(0, numberValue(ingredient.required_quantity) - numberValue(stock?.quantity));
  }
  ...
  async function ensureFoodShoppingList(){
    const existing=lists.find((list:any)=>list.category==='food-shopping') || lists.find((list:any)=>list.title.toLowerCase()==='compras de comidas');
  ...
  async function addShortageToShopping(meal:any,ingredient:any){
    if(!canEditLists||generatedIngredientIds.has(ingredient.id))return;
    const missing=shortage(ingredient); if(missing<=0)return;
  ...
  const missingIngredients=meals.reduce((sum,meal)=>sum+(meal.meal_ingredients||[]).filter((ingredient:any)=>shortage(ingredient)>0).length,0);
