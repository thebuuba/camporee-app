...
export default function ParticipantManager({ camporeeId, canEdit, initialParticipants }: { camporeeId: string; canEdit: boolean; initialParticipants: any[] }) {
  ...
  async function setPresence(person:any) {
    ...
    if (error) setParticipants((current) => current.map((item) => item.id === person.id ? {...item, attendance_status:person.attendance_status} : item));
  }
  ...
  const personCard = (person:any) => <article className="panel-row ios-card" key={person.id}>
    {rollCall ? <button className={`roll-call-check ${person.attendance_status === 'checked_in' ? 'present' : ''}`} onClick={() => setPresence(person)} disabled={!canEdit} aria-label={person.attendance_status === 'checked_in' ? 'Marcar ausente' : 'Marcar presente'}>{person.attendance_status === 'checked_in' ? <Check size={20}/> : <UserRound size={18}/>}</button> : <span className="stat-icon stat-blue"><UserRound size={18}/></span>}
    ...
    {!rollCall && canEdit ? <div className="row-actions"><button className="row-icon-btn" onClick={() => openEdit(person)} aria-label="Editar"><Pencil size={16}/></button><button className="row-icon-btn danger" onClick={() => removeParticipant(person.id)} aria-label="Eliminar"><Trash2 size={17}/></button></div> : null}
  </article>;
  ...
  return <>
    ...
    <div className="panel-tools">
      <label className="panel-search"><Search size={18}/><input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Buscar participante o unidad"/>{query ? <button onClick={() => setQuery("")} aria-label="Limpiar"><X size={16}/></button> : null}</label>
      ...
      {!rollCall && canEdit ? <button className="panel-add" onClick={openNew}><Plus size={18}/> Agregar participante</button> : null}
    </div>
