...
export default function AttendanceManager({ camporeeId, userId, canEdit, participants, initialSessions }: { camporeeId:string; userId:string; canEdit:boolean; participants:any[]; initialSessions:any[] }) {
  ...
  async function createSession(formData:FormData) {
    ...
    if (!title) return;
    setSaving(true);
    const payload = { camporee_id:camporeeId, title, session_type:String(formData.get('session_type') || 'general'), occurred_at:String(formData.get('occurred_at') || '') ? new Date(String(formData.get('occurred_at'))).toISOString() : new Date().toISOString(), created_by:userId };
    ...
    setSaving(false);
    if (!error && data) { const hydrated={...data,attendance_marks:[]}; setSessions((current)=>[hydrated,...current]); setActiveSession(hydrated); setOpen(false); router.refresh(); }
  }
  ...
  async function toggleParticipant(person:any) {
    ...
    if(error||!data){const rollback={participant_id:person.id,present:previous};setActiveSession((current:any)=>current&&current.id===sessionId?{...current,attendance_marks:updatePersonMark(current.attendance_marks||[],person.id,rollback)}:current);setSessions((current)=>current.map((session)=>session.id===sessionId?{...session,attendance_marks:updatePersonMark(session.attendance_marks||[],person.id,rollback)}:session));return;}
  ...
  async function markAllPresent() {
    ...
    if(error){setActiveSession((current:any)=>current&&current.id===sessionId?{...current,attendance_marks:previous}:current);setSessions((current)=>current.map((session)=>session.id===sessionId?{...session,attendance_marks:previous}:session));return;} router.refresh();
  }

  if (activeSession) return <>
    <div className='attendance-live-head ios-card'><div><small>{typeLabel(activeSession.session_type)}</small><h2>{activeSession.title}</h2><span>{presentCount}/{participants.length} presentes</span></div><button className='secondary-btn' onClick={()=>{setActiveSession(null);setQuery('')}}>Terminar</button></div>
