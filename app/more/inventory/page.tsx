...
export default async function InventoryPage(){
 ...
 const [{data:membership},{data:camporees}]=await Promise.all([supabase.from('app_members').select('role,is_active,permissions').eq('user_id',userId).maybeSingle(),supabase.from('camporees').select('id,status').order('starts_on',{ascending:true})]);
 if(!membership?.is_active)redirect('/login'); const permissions=(membership.permissions??{}) as Record<string,boolean>; const canEdit=membership.role==='admin'||membership.role==='editor'||Boolean(permissions.inventory); const camporee=camporees?.find(c=>c.status!=='archived')??camporees?.[0];
 const {data:items}=camporee?await supabase.from('inventory_items').select('*').eq('camporee_id',camporee.id).order('created_at',{ascending:false}):{data:[]};
