...
import { Award, CalendarCheck, CheckCircle2, Circle, MapPin, Medal, Pencil, Plus, Search, Trash2, Trophy, Users, X } from 'lucide-react';
...
const sortActivities = (a:any,b:any) => {
  if(Boolean(a.is_completed)!==Boolean(b.is_completed)) return a.is_completed ? 1 : -1;
  const byDate=(a.starts_at||'9999').localeCompare(b.starts_at||'9999');
...
export default function ActivityManager({ camporeeId, canEdit, initialActivities, participants }: { camporeeId:string; canEdit:boolean; initialActivities:any[]; participants:any[] }) {
  ...
  useEffect(() => { setActivities([...initialActivities].sort(sortActivities)); }, [initialActivities]);
  ...
  }).sort(sortActivities), [activities,participants,query,filter]);
  ...
  async function toggleComplete(activity:any){
    if(!canEdit) return;
    const next=!activity.is_completed;
    setActivities((current)=>current.map((item)=>item.id===activity.id?{...item,is_completed:next}:item).sort(sortActivities));
    const {error}=await supabase.from('camporee_activities').update({is_completed:next}).eq('id',activity.id);
    if(error){
      setActivities((current)=>current.map((item)=>item.id===activity.id?{...item,is_completed:activity.is_completed}:item).sort(sortActivities));
      reportMutationError(error,'No se pudo cambiar el estado de la actividad.');
      return;
    }
    reportMutationSuccess(next?'Actividad completada.':'Actividad reactivada.');
    router.refresh();
  }
  ...
  async function saveActivity(formData:FormData){
    ...
    try {
      ...
      const {data,error}=await request.select('id,title,activity_type,starts_at,location,responsible_name,materials,result,score,notes,schedule_event_id,is_completed,created_at').single();
      if(error || !data){
      ...
      setActivities((current)=>(editing ? current.map((item)=>item.id===data.id?hydrated:item) : [...current,hydrated]).sort(sortActivities));
      setOpen(false);
      setEditing(null);
      setFormError('');
