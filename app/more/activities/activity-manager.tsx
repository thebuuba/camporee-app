...
import { Award, MapPin, Medal, Pencil, Plus, Search, Trash2, Trophy, Users, X } from 'lucide-react';
...
export default function ActivityManager({ camporeeId, canEdit, initialActivities, participants }: { camporeeId:string; canEdit:boolean; initialActivities:any[]; participants:any[] }) {
  ...
  async function saveActivity(formData:FormData){
    ...
    const payload={
      ...
      starts_at:startsValue ? new Date(startsValue).toISOString() : null,
    ...
    const {data,error}=await request.select('id,title,activity_type,starts_at,location,responsible_name,materials,result,score,notes').single();
    ...
    const hydrated={...data,camporee_activity_participants:selected.map((participant_id)=>({participant_id}))};
  ...
  async function removeActivity(id:string){
    ...
    const {error}=await supabase.from('camporee_activities').delete().eq('id',id);
    if(!error){ setActivities((current)=>current.filter((item)=>item.id!==id)); router.refresh(); }
  }
