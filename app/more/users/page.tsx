...
export default async function UsersPage({ searchParams }: { searchParams: Promise<{ error?: string; saved?: string }> }) {
  ...
  return <main className="app users-app">
    <header className="subpage-top"><Link href="/more" className="back-btn" aria-label="Volver">‹</Link><div><div className="eyebrow">ADMINISTRACIÓN</div><h1>Usuarios y permisos</h1></div><div className="subpage-spacer" /></header>

    <section className="permission-intro ios-card"><span className="settings-icon red">👤</span><div><strong>Todos trabajan sobre el mismo camporee</strong><p>Controla quién puede administrar, editar o únicamente consultar. Los permisos se aplican también en Supabase, no solo en la interfaz.</p></div></section>

    {params.error ? <div className="auth-alert error">{params.error}</div> : null}
    {params.saved ? <div className="auth-alert success">Permisos actualizados.</div> : null}
    ...
    <section className="member-list">
      {(members ?? []).map((member) => {
        ...
        return <form action={updateMemberAccess} className="member-card ios-card" key={member.user_id}>
          ...
          <div className="permission-title"><strong>Permisos específicos</strong><small>Útiles sobre todo para cuentas de solo lectura.</small></div>
