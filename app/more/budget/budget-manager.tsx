...
export default function BudgetManager({ camporeeId, userId, canEdit, initialExpenses, initialIncome, areas }: BudgetManagerProps) {
  ...
  async function saveMovement(formData: FormData) {
    ...
    if (kind === 'expense') {
      const payload:any = {
        ...
        description,
        amount,
        spent_on: String(formData.get("movement_date") || new Date().toISOString().slice(0,10)),
      ...
      const payload:any = {
        ...
        description,
        amount,
        received_on: String(formData.get("movement_date") || new Date().toISOString().slice(0,10)),
        category: String(formData.get("category") || "").trim() || null,
  ...
  const movementDate = editing ? (kind === 'income' ? editing.received_on : editing.spent_on) : new Date().toISOString().slice(0,10);
