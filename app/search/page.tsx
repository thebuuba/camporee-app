...
import { Search, CheckCircle2, Users, CalendarDays, Utensils, ListChecks, StickyNote, FileText } from 'lucide-react';
...
export default async function SearchPage({ searchParams }: { searchParams: Promise<{ q?: string }> }) {
  ...
  const empty = { tasks: [], participants: [], events: [], meals: [], lists: [], notes: [], documents: [] } as any;
  let results = empty;
  if (camporee && term.length >= 2) {
    const pattern = `%${term.replace(/[%_]/g, '')}%`;
    const [tasks, participants, events, meals, lists, notes, documents] = await Promise.all([
    ...
    results = { tasks: tasks.data ?? [], participants: participants.data ?? [], events: events.data ?? [], meals: meals.data ?? [], lists: lists.data ?? [], notes: notes.data ?? [], documents: documents.data ?? [] };
  }
  const groups = [
  ...
  return <main className="app panel-page">
    ...
    <form className="panel-search" action="/search"><Search size={18}/><input name="q" defaultValue={term} placeholder="Tarea, persona, comida, documento…" autoFocus/><button type="submit" aria-label="Buscar">→</button></form>
    {!term ? <div className="empty compact">Escribe al menos dos letras para buscar en todo el camporee.</div> : term.length < 2 ? <div className="empty compact">Escribe un poco más para buscar.</div> : count === 0 ? <div className="empty compact">No encontré nada con “{term}”.</div> : groups.map(([title, Icon, rows, href, label]) => rows.length ? <section key={title} className="section-card ios-card search-results-group"><div className="section-head inside"><h3>{title}</h3><span>{rows.length}</span></div><div className="panel-list">{rows.map((row:any) => <Link href={href} className="panel-row search-result-row" key={row.id}><span className="stat-icon stat-green"><Icon size={17}/></span><div className="panel-row-copy"><strong>{label(row)}</strong><small>Ver en {title.toLowerCase()}</small></div><span className="chevron">›</span></Link>)}</div></section> : null)}
  </main>;
}
