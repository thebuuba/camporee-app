...
import { Check, Circle, ListChecks, Pencil, Plus, Search, Trash2 } from 'lucide-react';
...
export default function TaskManager({ camporeeId, userId, canEdit, initialTasks, areas }: { camporeeId: string; userId: string; canEdit: boolean; initialTasks: any[]; areas: any[] }) {
  ...
  const visible = useMemo(() => {
    ...
    return tasks.filter((task) => {
      const textOk = `${task.title} ${task.description ?? ''}`.toLowerCase().includes(query.toLowerCase());
  ...
  async function saveTask(formData: FormData) {
    ...
    if (!title) return;
    setSaving(true);
    const payload:any = { camporee_id:camporeeId, title, description:String(formData.get('description')||'').trim()||null, area_id:String(formData.get('area_id')||'')||null, priority:String(formData.get('priority')||'normal'), due_at:String(formData.get('due_at')||'')||null, phase:String(formData.get('phase')||'before') };
    ...
    const { data, error } = await queryBuilder.select('id,title,description,status,priority,due_at,area_id,phase').single();
    setSaving(false);
    if (!error && data) {
  ...
  return <>
    <div className='panel-tools'><label className='panel-search'><Search size={17}/><input value={query} onChange={(e)=>setQuery(e.target.value)} placeholder='Buscar tareas'/>{query?<button type='button' onClick={()=>setQuery('')}>×</button>:null}</label><div className='filter-chips'>{[['all','Pendientes'],['today','Hoy'],['urgent','Prioridad'],['done','Completadas']].map(([key,label])=><button key={key} className={filter===key?'active':''} onClick={()=>setFilter(key as any)}>{label}</button>)}</div>{canEdit?<button className='panel-add' onClick={()=>{setEditing(null);setOpen(true)}}><Plus size={18}/> Nueva tarea</button>:null}</div>
    {open?<div className='sheet-backdrop' onClick={()=>{setOpen(false);setEditing(null)}}><section className='sheet-card' onClick={(e)=>e.stopPropagation()}><div className='sheet-handle'/><h2>{editing?'Editar tarea':'Nueva tarea'}</h2><form action={saveTask} className='panel-form'><input name='title' defaultValue={editing?.title??''} placeholder='¿Qué hay que hacer?' required/><textarea name='description' defaultValue={editing?.description??''} placeholder='Detalles opcionales'/><select name='area_id' defaultValue={editing?.area_id??''}><option value=''>Sin área</option>{areas.map((area)=><option key={area.id} value={area.id}>{area.name}</option>)}</select><div className='form-two'><select name='priority' defaultValue={editing?.priority??'normal'}><option value='low'>Baja</option><option value='normal'>Normal</option><option value='high'>Alta</option><option value='urgent'>Urgente</option></select><select name='phase' defaultValue={editing?.phase??'before'}><option value='before'>Preparación</option><option value='during'>Durante</option><option value='after'>Regreso</option></select></div><input name='due_at' type='datetime-local' defaultValue={localInput(editing?.due_at)}/><button className='primary-btn' disabled={saving}>{saving?'Guardando…':editing?'Guardar cambios':'Crear tarea'}</button></form></section></div>:null}
