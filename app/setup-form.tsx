...
export default function SetupForm() {
  ...
  async function onSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    if (!name || !startsOn || !endsOn) {
      setError('Completa el nombre y las fechas del camporee.');
      return;
    }
    if (endsOn < startsOn) {
    ...
    try {
      ...
      if (!member?.is_active || member.role !== 'admin') {
        throw new Error('Solo un administrador puede crear el camporee.');
      }

      const { data: camporee, error: camporeeError } = await supabase
        .from('camporees')
        .insert({
          owner_id: userId,
          name,
      ...
      const { error: memberInsertError } = await supabase
        .from('camporee_members')
        .insert({ camporee_id: camporee.id, user_id: userId, role: 'admin' });
      if (memberInsertError) console.warn('camporee_members:', memberInsertError.message);

      const { error: areasError } = await supabase
        .from('areas')
        .insert(DEFAULT_AREAS.map(([areaName, icon], index) => ({
      ...
      setError(err instanceof Error ? err.message : 'Ocurrió un error al crear el camporee.');
      setLoading(false);
    }
  }
  ...
  return <form onSubmit={onSubmit} className="setup-form">
    ...
    <label>Nombre del camporee<div className="field-card"><span>✦</span><input name="name" placeholder="Ej. Firmes y Adelante 2026" required disabled={loading} /></div></label>
    <label>Lugar<div className="field-card"><span>⌖</span><input name="location" placeholder="Lugar del evento" disabled={loading} /></div></label>
