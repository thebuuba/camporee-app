...
import { AlertTriangle, CalendarDays, CheckCircle2, ChevronRight, ClipboardCheck, Clock3, DollarSign, Flag, Flame, MapPin, Search, ShoppingCart, StickyNote, TentTree, Users } from "lucide-react";
...
export default function HomeDashboard({ userId, firstName, avatarUrl, camporee, days, progress, pendingTasks, todayPendingTasks, participants, totalExpenses, phase, dayNumber, totalDays, programCount, todayProgramCount, currentEvent, nextEvent, todayTasks, urgentAnnouncement }: {
  ...
  return <main className={`app home-phase-${phase}`}>
    ...
    <section className={`camporee-state-card camporee-state-${phase}`}>
      <div className="camporee-state-main">
        <div className="camporee-state-top">
          ...
          <span className="camporee-state-logo" aria-hidden="true"><TentTree size={23}/></span>
        </div>
        <div className="camporee-tent-watermark" aria-hidden="true"><TentTree size={150}/></div>
        <div className="camporee-state-heading"><h2>{camporee.name}</h2>{camporee.location?<div className="camporee-state-location"><MapPin size={18}/><span>{camporee.location}</span></div>:null}</div>

        {active?<>
          <div className="live-day-card">
            <span className="camporee-card-label">Estamos en el</span>
            <div className="camporee-big-line"><div className="camporee-big-copy"><strong>Día {dayNumber}</strong><span>de {totalDays}</span></div><Flame className="camporee-big-icon" size={39}/></div>
          ...
          <div className="live-timeline-card">
            <div className="timeline-row now"><span className="timeline-marker"><Flame size={18}/></span><div className="timeline-copy"><small>AHORA</small><strong className={currentEvent?"":"empty"}>{currentEvent?.title??"Sin actividad programada"}</strong></div></div>
            <div className="timeline-row next"><span className="timeline-marker"><ChevronRight size={17}/></span><div className="timeline-copy"><small>PRÓXIMO</small><strong>{nextEvent?.title??"Fin del programa"}</strong></div>{nextEvent?<span className="timeline-time"><Clock3 size={14}/>{eventTime(nextEvent)}</span>:null}</div>
          </div>
        </>:after?<>
          ...
          <div className="prep-countdown-card">
            ...
            <div className="camporee-big-line"><div className="camporee-big-copy"><strong>Faltan {days}</strong><span>{days===1?"día":"días"}</span></div><TentTree className="camporee-big-icon" size={34}/></div>
          ...
          <div className="prep-status-card">
            <div className="timeline-row now"><span className="timeline-marker"><CheckCircle2 size={17}/></span><div className="timeline-copy"><small>AHORA</small><strong>Preparando el camporee</strong></div></div>
            <div className="timeline-row next"><span className="timeline-marker"><ChevronRight size={17}/></span><div className="timeline-copy"><small>PRÓXIMO</small><strong>Inicio del camporee</strong></div><span className="timeline-time">{days===0?"Hoy":days===1?"Mañana":`En ${days} días`}</span></div>
          </div>
        </>}
      </div>

      <div className="camporee-operation">
        <div className="operation-top"><span className="operation-ring" style={{background:`conic-gradient(#ff9f0a ${safeProgress*3.6}deg,#f2eee5 0)`}}><strong>{safeProgress}%</strong></span><div className="operation-copy"><small>{after?"Cierre":active?"Operación":"Preparación"}</small><strong>{after?safeProgress===100?"Todo cerrado":"Por cerrar":safeProgress===100?"Todo listo":`${safeProgress}% listo`}</strong></div><span className="operation-state"><Flag size={16}/>{active?"En marcha":after?"Finalizado":"Preparando"}</span></div>
        <div className="operation-meta"><span className="operation-chip"><CheckCircle2 size={15}/>{pendingTasks} {pendingTasks===1?"tarea pendiente":"tareas pendientes"}</span><span className="operation-chip"><Users size={15}/>{participants} {participants===1?"participante":"participantes"}</span></div>
      </div>
    </section>
