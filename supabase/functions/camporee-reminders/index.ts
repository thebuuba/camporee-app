...
function editorResponsibilities(permissions: PermissionMap | null | undefined) {
  return Object.entries(permissions ?? {})
    .filter(([key, enabled]) => enabled === true && permissionMeta[key])
    .map(([key]) => ({ key, ...permissionMeta[key] }))
...
function editorMessage(milestone: string, camporeeName: string, permissions: PermissionMap | null | undefined): ReminderMessage {
  ...
  if (milestone === "d10") return {
    title: "🏕️ Faltan 10 días",
    body: `Faltan 10 días para ${camporeeName}. Revisa ${focus}${extra} y detecta lo que todavía falta preparar.`,
    url,
  };
  if (milestone === "d5") return {
    title: "🔥 Ya casi estamos",
    body: `Quedan 5 días. Confirma ${focus}${extra} y deja resueltos los pendientes de tus áreas.`,
    url,
  };
  if (milestone === "d1") return {
    title: "⛺ Mañana comenzamos",
    body: `Mañana comienza ${camporeeName}. Deja listos ${focus}${extra} antes de salir.`,
    url,
  };
  return {
    title: "🎉 ¡Llegó el día!",
    body: `¿Estás listo? Hoy comienza ${camporeeName}. Mantén al día ${focus}${extra} durante la operación.`,
    url,
  };
}
...
Deno.serve(async (req) => {
  try {
    ...
    const targets = (camporees ?? [])
      .map((camporee) => ({ ...camporee, daysLeft: daysBetween(today, camporee.starts_on) }))
